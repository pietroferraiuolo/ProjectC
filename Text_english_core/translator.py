...
Notes
-----
- The user may want to edit the `base_path` variable to point to the correct
    folder where the text files are stored. The default path is set by checking
    whhether it is executed on a Windows or Linux system, but it can be changed
    to a default path.
- The script is meant to be used with the text files from the Pokémon Essentials
    project, which are formatted in a specific way. The script reads the file
    line by line, asking for the translation of each line. The translation is
    saved in a new file with the same name as the original file, but with the
    language code appended at the end.
- The script is meant to be used in conjunction with the `Main` script, which
    lists the files in the `Text_english_core` folder.
"""
...
def translate_file(file_name, language="it"):
    ...
    file_translation = os.path.join(
        base_path, f"{file_name.split('.')[0]}_{language}.txt"
    )
    last_transl_line = 0
    if os.path.exists(file_translation):
        print(
            f"File `{file_translation.split('/')[-1]}` already exists. Do you want to load it? (y/n)",
            end=" ",
        )
        choice = input()
        if choice == "n":
    ...
    start_line = input()
    if start_line == "":
        last_true_transl_line = (
            (last_transl_line + 1) * 2 if last_transl_line > 0 else 0
        )
    ...
    for i in range(start_line, len(lines), 2):
        ...
        if n >= start_line:
            ...
            if translation.lower() in ["exit", "quit", "q"]:
                lines.insert(
                    0, f"# Translation stopped at line {maximum(n_shown-1,0)}\n"
                )
                break
            if translation == "":
                translation = line
...
if __name__ == "__main__":
    ...
    if OS == "Windows":
        ...
        base_path = os.path.join(
            os.path.expanduser("~"), "git", "ProjectC", "Text_english_core"
        )
